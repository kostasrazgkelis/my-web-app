...
class _ToolsAndFrameworksSectionState extends State<ToolsAndFrameworksSection> {
  ...
  void initState() {
    ...
    showContainerOrchestrationChips = List.filled(widget.containerOrchestration.length, false);
    ...
    showOperatingSystemsChips = List.filled(widget.operatingSystems.length, false);
  ...
  void _startSequentialAnimation() async {
    ...
    await _animateChips(widget.containerOrchestration, showContainerOrchestrationChips);
  ...
  Future<void> _animateChips(List<Map<String, dynamic>> items, List<bool> showFlags) async {
      ...
      child: Column(
        ...
        children: [
          ...
          if (showLanguagesSection) ...[
            _buildSubsection('Languages', widget.programmingLanguages, showLanguagesChips),
            const SizedBox(height: 24),
          ],
          
          // Frameworks Section
          if (showFrameworksSection) ...[
            _buildSubsection('Frameworks', widget.frameworks, showFrameworksChips),
            const SizedBox(height: 24),
          ],
          
          // Version Control Section
          if (showVersionControlSection) ...[
            _buildSubsection('Version Control', widget.versionControl, showVersionControlChips),
            const SizedBox(height: 24),
          ],
          
          // Data Processing & Big Data Section
          if (showDataProcessingSection) ...[
            _buildSubsection('Data Processing & Big Data', widget.dataProcessing, showDataProcessingChips),
            const SizedBox(height: 24),
          ],
          
          // Container Orchestration Section
          if (showContainerOrchestrationSection) ...[
            _buildSubsection('Container Orchestration Platforms', widget.containerOrchestration, showContainerOrchestrationChips),
            const SizedBox(height: 24),
          ],
          
          // Databases Section
          if (showDatabasesSection) ...[
            _buildSubsection('Databases & Data Stores', widget.databases, showDatabasesChips),
            const SizedBox(height: 24),
          ],
          
          // CI/CD & DevOps Section
          if (showCicdDevOpsSection) ...[
            _buildSubsection('CI/CD & DevOps Pipelines', widget.cicdDevOps, showCicdDevOpsChips),
            const SizedBox(height: 24),
          ],
          
          // Operating Systems Section
          if (showOperatingSystemsSection) ...[
            _buildSubsection('Operating Systems', widget.operatingSystems, showOperatingSystemsChips),
          ],
        ],
      ),
    );
  ...
  Widget _buildSubsection(String title, List<Map<String, dynamic>> items, List<bool> showFlags) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
